...
export function TransactionsUI({ initial }: { initial: Tx[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter(
        (t) => t.description?.toLowerCase().includes(s) || t.id.toLowerCase().includes(s)
      );
    }
    if (range.start) data = data.filter((t) => new Date(t.createdAt) >= new Date(range.start!));
    if (range.end)
      data = data.filter((t) => new Date(t.createdAt) <= new Date(range.end! + "T23:59:59"));
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        >
          <div className="text-sm">
            ...
            <button onClick={() => setType(null)} className="mt-3 text-xs text-subt hover:underline">
              Clear
            </button>
          </div>
        </Popover>
              ...
              {ALL_COLUMNS.map((c) => (
                <div key={c.key} className="flex items-center justify-between py-1">
                  ...
                  <Toggle
                    ...
                    onChange={(v) => setColumns((s) => ({ ...s, [c.key]: v }))}
                  />
                </div>
              ))}
            </div>
      ...
      <div className="bg-panel rounded-xl2 border border-outline/40 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-subt">
              {visibleCols.map((c) => (
                <th
                  key={c.key}
                  className="text-left font-normal px-4 py-3 border-b border-outline/30"
                >
                  {c.label}
                </th>
              ))}
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td className="px-4 py-10 text-center text-subt" colSpan={visibleCols.length}>
                  No transactions
                </td>
              </tr>
            ) : (
              filtered.map((t) => (
                <tr key={t.id} className="hover:bg-surface/50">
                  {visibleCols.map((c) => {
                    ...
                    return (
                      <td key={key} className="px-4 py-3 border-b border-outline/20 align-top">
                        {value}
                      </td>
                    );
                  })}
