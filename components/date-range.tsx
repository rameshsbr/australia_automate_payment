...
export function DateRangeFilter({ value, onChange }:{ value:DateRange; onChange:(v:DateRange)=>void; }) {
  ...
  return (
    <Popover align="left" className="w-[340px]"
      button={({open})=>(
        <div className="inline-flex items-center gap-2 bg-panel border border-outline/40 rounded-lg px-3 h-9 text-sm">
          <span>Date</span><span className="text-subt">{value.start&&value.end?`${value.start} to ${value.end}`:"Custom"}</span>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <input type="date" className="bg-panel border border-outline/40 rounded px-2 h-9 text-sm"
        ...
        <button onClick={()=>onChange(draft)} className="w-full bg-[#6d44c9] rounded h-9 text-sm">Apply</button>
      </div>
    </Popover>
  );
}
