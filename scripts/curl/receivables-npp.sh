...
START="${1:?usage: receivables-npp.sh <YYYY-MM-DD> <YYYY-MM-DD>}"
END="${2:?usage: receivables-npp.sh <YYYY-MM-DD> <YYYY-MM-DD>}"
ENV="${ENV:-sandbox}"
...
normalize_dupe_end() {
  # collapse accidental .../<end>/<end> tail
  local path="$1"
  local a="${path%/}"             # strip trailing /
  local last="${a##*/}"
  local prev="${a%/*}"; prev="${prev##*/}"
  if [[ "$last" == "$prev" ]]; then
    printf '%s' "${a%/*}"         # drop last seg
  else
    printf '%s' "$a"
  fi
}
...
if [[ "${DIRECT:-0}" == "1" ]]; then
  ...
  TMPL="${MONOOVA_PATH_REPORT_RECEIVABLES_NPP:-/receivables/v1/npp/{startDate}/{endDate}}"
  ...
  curl -sS "${BASE}/api/monoova/financial/reports/receivables/npp/${START}/${END}?env=${ENV}" | jq .
fi