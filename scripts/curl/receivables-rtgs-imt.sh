...
START="${1:?usage: receivables-rtgs-imt.sh <YYYY-MM-DD> <YYYY-MM-DD>}"
END="${2:?usage: receivables-rtgs-imt.sh <YYYY-MM-DD> <YYYY-MM-DD>}"
ENV="${ENV:-sandbox}"
...
if [[ "${DIRECT:-0}" == "1" ]]; then
  ...
  TMPL="${MONOOVA_PATH_REPORT_RECEIVABLES_RTGS_IMT:-/receivables/v1/rtgs-imt/{startDate}/{endDate}}"
  ...
  curl -sS "${BASE}/api/monoova/financial/reports/receivables/rtgs-imt/${START}/${END}?env=${ENV}" | jq .
fi